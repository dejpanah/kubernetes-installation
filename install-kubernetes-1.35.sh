...
sudo apt install -y curl gnupg2 gpg software-properties-common apt-transport-https ca-certificates >/dev/null 2>&1
...
curl -fsSL https://pkgs.k8s.io/core:/stable:/v1.35/deb/Release.key | sudo gpg --dearmour -o /etc/apt/keyrings/kubernetes-apt-keyring.gpg
